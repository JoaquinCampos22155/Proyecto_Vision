...
class ViewTypeService:
    def estimate(self, image: Image.Image, image_index: int, filename: str = "") -> ViewTypeResult:
        ...
        if "back" in name or "espalda" in name:
            return ViewTypeResult(image_index, "main_back", 0.72)
        if "front" in name or "frente" in name:
            return ViewTypeResult(image_index, "main_front", 0.72)
        if "collar" in name or "cuello" in name:
            return ViewTypeResult(image_index, "detail_collar", 0.74)
        if "logo" in name:
            return ViewTypeResult(image_index, "detail_logo", 0.74)
        if "label" in name or "tag" in name or "etiqueta" in name:
            return ViewTypeResult(image_index, "detail_label", 0.74)
        if "fabric" in name or "tela" in name:
            return ViewTypeResult(image_index, "detail_fabric", 0.70)
        ...
        if foreground_ratio >= 0.72 and edge_density >= 0.17:
            return ViewTypeResult(image_index, "detail_fabric", 0.58)

        if foreground_ratio >= 0.62 and edge_density >= 0.11:
            return ViewTypeResult(image_index, "detail_logo", 0.56)

        if foreground_ratio >= 0.52 and center_foreground_ratio >= 0.50:
            return ViewTypeResult(image_index, "partial_view", 0.60)

        if foreground_ratio >= 0.34:
            return ViewTypeResult(image_index, "main_front", 0.55)

        if foreground_ratio >= 0.20:
            return ViewTypeResult(image_index, "partial_view", 0.48)

        return ViewTypeResult(image_index, "unknown", 0.35)
